...
Абзац, начинающийся с "+ ", выделяется тегом \<li> в элемент списка.
В тексте элемента списка могут присутствовать все внутристрочные символы разметки с указанными правилами.
Элементы списка объединяются в неупорядоченный список \<ul>.

Таким образом

+ элемент __с _разными_ символами__
+ другой элемент __с _разными_ символами__
 
 Превратится в:

 \<ul>  
 \<li>элемент \<strong>с \<em>разными\</em> символами\</strong>\</li>  
 \<li>другой элемент \<strong>с \<em>разными\</em> символами\</strong>\</li>  
 \</ul>
