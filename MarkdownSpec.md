...
Абзац, начинающийся с "* ", выделяется тегом <li>.
В тексте элемента списка могут присутствовать все прочие символы разметки с указанными правилами, кроме заголовков.

# Взаимодействие элементов списка
Если несколько подряд абзацев начинаются с маркированного списка, эти абзацы с элементами списка объединяются тегом <ul>.
Одиночные абзацы с элементом маркированного списка, также оборачиваются в <ul>.

Таким образом

# Маркированный список __с _разными_ символами__

превратится в:

<ul><li>Маркированный список <strong>с <em>разными</em> символами</strong></li></ul>

а несколько идущих подряд:

# * Элемент списка\n* Элемент списка

превратятся в:

<ul><li>Элемент списка</li>\n<li>Элемент списка</li></ul>