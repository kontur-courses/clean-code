...
Списком считается непрерывная последовательность ListItem, где ListItem = "* текст\n".
Если после * нет пробела, то это не ListItem => список не начинается, либо прерывается
Если строка состоит только из * , то это не ListItem => список не начинается, либо прерывается
Список прерывается абзацами, то есть:
* Первый
* Список
...
В элементах списка поддерживаются _текст_ и __текст__.