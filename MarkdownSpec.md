...
Для создания ссылки используется следующий синтаксис [Label](URL).
Такая конструкция превращается в:
<a href="URL">Label</a>

Другие теги не могу находится внутри ссылки, но могут обрамлять её.
Так _[Label](URL)_ превратится в:
