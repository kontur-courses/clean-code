...
* Тег должен начинаться с самого начала текста или с нового абзаца, то есть после \n 
    примеры: \n* слово или * слово
* Тег должен заканчиваться при \n или при завершении всего текста           
    примеры: конце \n или конце пусто
* Маркированный список не должен находится внутри заголовка
* Конвертированный список должен находится внутри html тегов <ul> </ul>
    примеры: * это в одном списке \n \n* это в другом \n => <ul><li> это в одном списке </li></ul> \n<ul><li> это в другом </li></ul>
* Абзац, начинающийся с "*", выделяется тегом \<li> в элемент, а конец элемента списка заканчивается на </li>
* В случае если между элементами списка находится перенос строки, то есть при случаи \n \n, то должно происходит завершение текущего списка и создания следующего, если через абзац есть элемент списка. Оба списка должны быть выделены тегами <ul> </ul>
