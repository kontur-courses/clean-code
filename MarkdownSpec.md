...
Текст, ограниченный символами "*" будет преобразован в тег \<ul>.
При этом весь список должен находиться в одном абзаце.
Список не может находиться внутри заголовка.
Внутри этого тега распознаются только теги элемента списка, все
остальные выделения считаются обычными символами.

# Элемент списка

Текст, ограниченный символами "+" будет преобразован в тег \<li>.
Элемент списка не может находиться внутри заголовка.
Если элемент списка находиться не внутри списка, то он не распознается как тег.
