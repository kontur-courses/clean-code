...
Абзац, начинающийся с "* " выделяется тегом \<li> в элемент неупорядоченного списка, перед которым будет поставлен маркер.
В элементе списка может присутствовать _курсив_ и __полужирное начертание__.
Элементы списка оборачиваются в теги \<ul> и \</ul>. Эти теги обозначают собой начало и конец списка соответственно.
Markdown-версия разметки для корректной работы требует "\n" перед и после каждой строки с тегом "* ".

Как пример:

Текст\n* _с курсивом внутри листа_\n* __и полужирным внутри листа__\nи текстом снаружи

будет превращён в:

Текст\<ul>\<li>\<em>с курсивом внутри листа\</em>\</li>\<li>\<strong>и полужирным внутри листа\</strong>\</li>\</strong>\</ul>и текстом снаружи