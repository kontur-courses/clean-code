...
public static class StringExtensions
{
    ...
    public static bool IsInBound(this string str, int index) => index >= 0 && index < str.Length;

    public static bool IsRangeInBound(this string str, int start, int length) => length > 0 && str.IsInBound(start) && str.IsInBound(start + length - 1);

    public static bool HasUnescapedCharAt(this string str, int index, char c)
    {
        return IsInBound(str, index) &&  str[index] == c && EscapeRules.IsNotEscaped(str, index);
    }
