...
Абзац начинающийся с символов *, - или + выделится тегами \<ul> для начала целого списка и \<li> для каждого элемента списка

Процессор относит к разным спискам пункты, перед которыми стоят разные маркеры. Даже несмотря на то, что мы не оставляем пустых строк между списками.

Если же два списка идут подряд, а перед их пунктами стоят одинаковые маркеры, тогда между ними можно отбить две пустые строки, тогда они разобьются на отдельные списки.
