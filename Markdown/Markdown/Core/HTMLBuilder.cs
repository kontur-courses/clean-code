...
namespace Markdown
{
    ...
    {
        public static string Build(IEnumerable<Token> tokens, string text, IEnumerable<MarkTranslatorElement> marks,
        ...
        private static string AppendTaggedToken(string text, Token token)
        {
            ...
            var sortedStartsAndEnds = tokens
                .SelectMany(t => new TagInserter[]
                {
                    new TagInserter(t.StartIndex - t.MarkToTranslate.From.Opening.Length,
                        t.MarkToTranslate.To.Opening, t.MarkToTranslate.From.Opening),
                    new TagInserter(t.EndIndex + 1, t.MarkToTranslate.To.Closing, t.MarkToTranslate.From.Closing)
                })
                .OrderByDescending(item => item.Index);
            foreach (var pair in sortedStartsAndEnds)
            {
                builder.Remove(pair.Index + offset, pair.Mark.Length);
                builder.Insert(pair.Index + offset, pair.Tag);
            }
        ...
        private static string RemoveRedundantEscapeCharacters(string text, IEnumerable<MarkTranslatorElement> marks,
            char escapeCharacter)
        {
            ...
            foreach (var oneSymbolMark in marks.Where(m => m.From.Opening.Length == 1))
            {
                ...
                while (curIndex < builder.Length)
                {
                    ...
                    {
                        if (builder[curIndex].ToString() == oneSymbolMark.From.Opening &&
                            escapeCharactersCount % 2 == 1)
                        {
